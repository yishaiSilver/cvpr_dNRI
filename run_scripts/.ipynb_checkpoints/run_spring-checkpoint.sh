...
DATA_PATH='data/spring/'

BASE_RESULTS_DIR="results/spring/"
...
# mkdir -p $WORKING_DIR
# CUDA_VISIBLE_DEVICES=$GPU python -u dnri/experiments/spring_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS | tee "${WORKING_DIR}results.txt"
# CUDA_VISIBLE_DEVICES=$GPU python -u dnri/experiments/spring_experiment.py --gpu --mode eval --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS | tee "${WORKING_DIR}eval_results.txt"
# MODEL_ARGS="--model_type nri --graph_type dynamic ${ENCODER_ARGS} ${DECODER_ARGS}"
# CUDA_VISIBLE_DEVICES=$GPU python -u dnri/experiments/spring_experiment.py --gpu --mode eval --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS --error_out_name prediction_errors_dynamic.npy | tee "${WORKING_DIR}eval_results_dynamic.txt"
...
TRAINING_ARGS='--batch_size 256 --lr 5e-4 --use_adam --num_epochs 100 --normalize_kl --normalize_nll --tune_on_nll --val_teacher_forcing --teacher_forcing_steps -1'
mkdir -p $WORKING_DIR
CUDA_VISIBLE_DEVICES=$GPU python -u dnri/experiments/spring_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS | tee "${WORKING_DIR}results.txt"
CUDA_VISIBLE_DEVICES=$GPU python -u dnri/experiments/spring_experiment.py --gpu --mode eval --load_best_model --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS | tee "${WORKING_DIR}eval_results.txt"
